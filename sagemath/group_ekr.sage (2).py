...
def cayleygraph(G):
    ...
    X = Graph(X)
    Y = set(X.independent_set()) 
    print Y
    a ='False'
    for i in G.domain():
        if set(G.stabilizer(i)) == Y:
            a = 'true',set(G.stabilizer(i)),Y
        else:
            a = 'false',set(G.stabilizer(i)),Y
    return len(X.independent_set()),G.stabilizer(_sage_const_1 ).cardinality(),a,G.group_id()
...
#    print cayleygraph(H),H.is_transitive()

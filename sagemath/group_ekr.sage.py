...
def cayleygraph(G):
    ...
    return len(Z),G.stabilizer(G.domain()[_sage_const_1 ]).cardinality(),X.clique_number(algorithm = "MILP")#,Z,G.group_id()#,a
...
def HS():
    ...
    for H in All:
        if H.order() == A.order()/_sage_const_2 :
...
#X = Graph(X)
X = graphs.CycleGraph(_sage_const_3 ).disjoint_union(graphs.CycleGraph(_sage_const_3 ))
X.relabel()

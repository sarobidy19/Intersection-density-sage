...
# This file was *autogenerated* from the file ./no-sekr.sage
from sage.all_cmdline import *   # import sage library

_sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_6 = Integer(6); _sage_const_4 = Integer(4); _sage_const_8 = Integer(8)#!/usr/local/bin/sage
...
def max_clq(L):
    for X in L[_sage_const_0 ]:
        ...
        #    s = s*i
        #print s
        if L[_sage_const_1 ] != _sage_const_0 : #s != x.inverse()*x:
           if PermutationGroup(S).order() != len(S): #test whether S is a group or not
              print S
           print "size", L[_sage_const_2 ]

def test():
    G = graphs.CycleGraph(_sage_const_4 ).disjoint_union(graphs.CycleGraph(_sage_const_4 ))
    G.relabel()
...
def test1():
    ...
    #E = X.eigenspaces()
    #print X.eigenspaces()
    print X.independent_set(value_only=True),A.stabilizer(_sage_const_1 ).order(),A.is_transitive(),X.clique_number()#,X.chromatic_number()##,X.order()/X.clique_number()#,float(X.order()/(1-(max(E)/min(E))))
    B = X.independent_set()
    C = []
    y = B[_sage_const_0 ].inverse()
    for x in B:
        C.append(y*x)
...
def test2():
    ...
    for G in L:
        ...
        #E = X.eigenspaces()
        #print E
        print X.independent_set(value_only=True),A.stabilizer(_sage_const_1 ).order(),A.is_transitive()#,X.chromatic_number()#G.clique_number()#,X.order()/X.clique_number()#,float(X.order()/(1-(max(E)/min(E))))


def main():
    G = CyclicPermutationGroup(_sage_const_8 )
    G0 = list(G)
    G0.pop(_sage_const_0 )
    i = _sage_const_0 
    for A in Subsets(G0,_sage_const_2 ):
        i += _sage_const_1 
        print i
        A = list(A)
        if A[_sage_const_0 ]*A[_sage_const_1 ] == A[_sage_const_0 ].inverse()*A[_sage_const_0 ]:
            L = circulant_graph(G,A)
            max_clq(L)

def main1():
    G = graph.CycleGraph(_sage_const_6 ).disjoint_union(graphs.CycleGraph(_sage_const_6 ))
    A = G.automorphism_group()

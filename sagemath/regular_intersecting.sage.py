...
# This file was *autogenerated* from the file ./regular_intersecting.sage
...
def is_derangement(x):  # for 2 intersecting
    return len(Permutation(x).fixed_points()) == _sage_const_0 
...
def degreeF(G,F):
    m = G.degree()
    return [degree(i,F) for i in range(_sage_const_1 ,m+_sage_const_1 )]

def normalize(F):
    return [F[_sage_const_0 ].inverse()*g for g in F]
...
for F in Z:
    v = degreeF(G,F)
    if sum(v) == v[_sage_const_0 ]*n and sum(v) == v[_sage_const_1 ]*n and sum(v) == v[_sage_const_2 ]*n and sum(v) == v[_sage_const_3 ]*n:
      print F,v, len(F)

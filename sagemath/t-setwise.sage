...
"""def is_derangement(x):  # for 3 setwise intersecting
    if len(Permutation(x).fixed_points()) > 2:
          return False
    else:
          if len(Permutation(x).fixed_points()) == 0:
                return True
          elif len(Permutation(x).fixed_points()) == 1 and 3 not in Permutation(x).fixed_points():
                return True
          elif len(Permutation(x).fixed_points()) == 2 and 3 not in Permutation(x).fixed_points():
                return True
          else:
              return False"""
...
def weights(n,L,M):
    ...
    for chi in char:
        ...
        if phi.values() == [1]*len(SymmetricGroup(n).conjugacy_classes_representatives()):
            Eq.append(t == factorial(3)*binomial(n,3)-1)  ######## maximum eigenvalue
        else:
...
#Alt = alt_char(n)
#print Alt
W = weights(n,[[n],[n-1,1],[n-2,2],[n-2,1,1],[n-3,3],[n-3,2,1],[n-4,2,2]],[[n],[n-1,1],[n-2,2],[n-2,1,1],[n-3,3],[n-3,1,1,1],[n-3,2,1]])
